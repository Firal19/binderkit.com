...
${rail}
${station(1, 'count', node('The count').label + '.', `${reg('count')}${reg('scan')}${para(node('The count').note)}<div class="shows">${showMe('Rice · 10 lb', 'The row that is short')}${showMe('Bottled water', 'The reserve gap')}</div>`, phone('stock'))}
${station(2, 'queue', 'The queue.', `${reg('queue')}${para(node('The shortfall').note)}<div class="shows">${buy.rows.slice(0, 4).map((r) => showMe(r.item, r.origin.split(' · ')[0])).join('')}</div>`, phone('buy'), { rev: true, gist: 'What to buy and nothing else, each line with its origin.' })}
...
${station(4, 'shop', node('The shop').label + '.', `${reg('shop')}${para(node('The shop').note)}${para((find('objections').rows.find(([q]) => /signal/.test(q)) || ['', ''])[1])}<div class="shows">${showMe('Oatmeal', 'Still to pick')}${showMe('Bottled water', 'Cheaper at Costco')}</div>`, phone('shop'), { rev: true, gist: 'Aisle order at that shop. It works with no signal.' })}
...
${sec('token', 'token-s', `<div class="wrap">
  <div class="head">${sticker('lock', 'The sixth person has no account')}${h2('token', 'A spouse doing the Costco run.', 'The person doing the shopping is often not the person with the licence. A link opens one run, and nothing else.')}</div>
  <div class="tok">
    ...
    <div class="tok-c is-no"><span class="strip-l">${ic('x', 16)}It grants nothing else</span><ul>${['No session, and no route into anything else', 'No resident, no tag, no tray note', 'No write to the ledger until a signed-in person confirms'].map((t) => `<li>${esc(t)}</li>`).join('')}</ul></div>
  </div>
  <p class="st-p">${esc(sharedRun.text)}</p>
